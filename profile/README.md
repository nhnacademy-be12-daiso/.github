...
### 도서
- 하이브리드 도서 등록
  - 수동 등록: 관리자가 직접 도서 메타데이터를 입력하고 TUI Editor로 상세 내용을 편집하여 등록 가능
  - ISBN 기반 자동 완성: 알라딘 Open API를 연동하여 ISBN 입력 시 기본 정보를 실시간으로 수집하고, Gemini AI를 통해 도서 정보 가공 및 추가하여 관리자의 입력 공수 대폭 절감
  - 데이터 검증 프로세스: AI가 자동 생성한 정보를 관리자가 최종 확인 및 수정 후 등록할 수 있는 검증단계 구현
  - 멀티 포맷 이미지 프로세싱: 도서 커버 및 상세 이미지를 원본 파일(MultipartFile) 또는 외부 URL 형식으로 유연하게 수신하고, 등록 시점에 MinIO로 통합 이전하여 데이터 영속성 확보
- 도서 정보 관리 최적화
  - 소프트 삭제 적용: `isDeleted` 필드를 활용해 데이터 무결성을 유지하며 사용자에게는 404 응답 처리
  - `BatchSize` 및 Projection 도입: 다중 일대다 관계 조회 시 발생하는 N+1 문제를 방지하고 필요한 필드만 조회하여 성능 최적화

### 카테고리
- 계층형 카테고리 아키텍처 설계
    - 재귀적 쿼리를 지양하고 트리 구조(Tree Structure) 변환 로직을 구현하여 프론트엔드 네비게이션 최적화
    - 프론트엔드 서버 Redis 캐싱 전략: 카테고리 데이터 조회 속도 향상을 위한 캐싱 및 수정 발생 시 실시간 동기화 로직 구현
- 도서 등록 최적화: 말단 카테고리(Leaf Category)만 선택 가능하도록 필터링하여 데이터 일관성 확보

### 이미지
- MinIO 기반 오브젝트 스토리지 연동
    - 리뷰 미디어 관리: 주문 연동형 리뷰 등록 시 사용자의 원본 이미지 파일을 수신하여 MinIO에 안전하게 적재
    - 데이터 내재화: 외부 URL 소스를 서버에서 실시간 다운로드하여 내부 저장소로 이전함으로써 외부 의존성 제거
    - 조회 최적화: 보안을 위한 Proxy 요청 방식 도입 및 브라우저 캐싱 활용으로 이미지 로딩 성능 개선
...
### 좋아요
- 로그인 기반 개인화 서비스
    - 도서 목록/상세 조회 시 사용자의 '좋아요' 여부를 실시간 매핑하여 제공
    - 비동기 좋아요 등록/취소 및 사용자별 좋아요 도서 리스트 페이지 구현

### 리뷰
- 주문 연동형 리뷰 시스템
    - 주문 서비스와의 통신을 통해 '배송 완료' 상태의 주문 상세 건에 대해서만 리뷰 작성 권한 부여
    - 모달(Modal) UI 기반의 직관적인 리뷰 등록·조회 프로세스 구현
- 상세 도서 페이지 리뷰 조회
