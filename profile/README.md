...
- MinIO 기반 오브젝트 스토리지 연동
    - 모든 도서 및 리뷰 이미지를 중앙 집중식 저장소(MinIO)에 등록 및 관리
    - Image Migration Tool 개발: 외부 도서 API의 이미지 URL을 로컬 스토리지로 일괄 마이그레이션하는 배치 기능 구현
- 조회 성능 최적화
    - 프론트엔드 Proxy 요청 방식을 통한 보안적인 이미지 다운로드 흐름 구축
    - 브라우저 캐싱(Browser Caching) 전략을 통한 중복 조회 부하 최소화
